...
function requestDM(i) {
	...
	$.ajax({
		...
		success : function(ret) {
			...
			//videos[i].count = ret.views_last_hour;
			ready();
			setDesc(i);
		},
		error : function(ret) {
...
function load(video) {
	if (video.type == 'dm') {
		...
		loadOnLS(video.id);
	} else if (video.type == 'jtv') {
		loadOnJTV(video.id);
	} else if (video.type == 'o3d') {
		loadOnO3D(video.id);
	}
